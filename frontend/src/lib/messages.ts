...
const stateCopy: Record<TaskState, { content: string; tone?: ChatMessage['tone'] }> = {
  PENDING: { content: 'Task queued.' }, ANALYZING: { content: 'Parent is selecting and running specialists.' }, DECIDING: { content: 'Agents are evaluating options.' }, PLANNING: { content: 'Planner is generating an execution plan.' }, POLICY_CHECK: { content: 'Checking tool policy and execution risk.' }, WAITING_CONFIRMATION: { content: 'Human confirmation is required.', tone: 'warning' }, EXECUTING: { content: 'Worker is executing the approved plan.' }, VERIFYING: { content: 'Reviewer is verifying execution.', }, REPLANNING: { content: 'Planner is adjusting the plan.', tone: 'warning' }, NEEDS_REVIEW: { content: 'Task needs human review.', tone: 'warning' }, SUCCEEDED: { content: 'Task completed.', tone: 'success' }, FAILED: { content: 'Task failed.', tone: 'error' }, CANCELLED: { content: 'Task cancelled.', tone: 'warning' }, REJECTED: { content: 'Task rejected by policy.', tone: 'error' }, BUDGET_EXCEEDED: { content: 'Task budget exceeded.', tone: 'error' },
}
