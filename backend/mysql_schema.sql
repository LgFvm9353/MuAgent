...
CREATE TABLE IF NOT EXISTS `confirmations` (
    ...
    `turn_id` CHAR(32) NULL,
    `tool_call_id` CHAR(32) NULL,
    `call_hash` VARCHAR(64) NOT NULL,
    `status` VARCHAR(32) NOT NULL DEFAULT 'decided',
    ...
    UNIQUE KEY `uq_confirmations_tool_call_hash` (`tool_call_id`, `call_hash`),
    KEY `ix_confirmations_task_id` (`task_id`),
    KEY `ix_confirmations_plan_id` (`plan_id`),
    KEY `ix_confirmations_turn_id` (`turn_id`),
    KEY `ix_confirmations_tool_call_id` (`tool_call_id`),
    CONSTRAINT `fk_confirmations_task_id`
        FOREIGN KEY (`task_id`) REFERENCES `tasks` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_confirmations_plan_id`
        FOREIGN KEY (`plan_id`) REFERENCES `execution_plans` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_confirmations_turn_id`
        FOREIGN KEY (`turn_id`) REFERENCES `conversation_turns` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_confirmations_tool_call_id`
...
CREATE TABLE IF NOT EXISTS `evidence_records` (
    ...
    KEY `ix_evidence_records_turn_id` (`turn_id`),
    KEY `ix_evidence_records_agent_run_id` (`agent_run_id`),
    KEY `ix_evidence_records_tool_call_id` (`tool_call_id`),
    CONSTRAINT `fk_evidence_records_task_id`
        FOREIGN KEY (`task_id`) REFERENCES `tasks` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_evidence_records_step_id`
        FOREIGN KEY (`step_id`) REFERENCES `execution_steps` (`id`),
    CONSTRAINT `fk_evidence_records_turn_id`
        FOREIGN KEY (`turn_id`) REFERENCES `conversation_turns` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_evidence_records_agent_run_id`
        FOREIGN KEY (`agent_run_id`) REFERENCES `agent_runs` (`id`) ON DELETE SET NULL,
    CONSTRAINT `fk_evidence_records_tool_call_id`
...
CREATE TABLE IF NOT EXISTS `audit_events` (
    ...
    KEY `ix_audit_events_turn_id` (`turn_id`),
    KEY `ix_audit_events_agent_run_id` (`agent_run_id`),
    KEY `ix_audit_events_tool_call_id` (`tool_call_id`),
    KEY `ix_audit_events_trace_id` (`trace_id`),
    CONSTRAINT `fk_audit_events_task_id`
        FOREIGN KEY (`task_id`) REFERENCES `tasks` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_audit_events_turn_id`
        FOREIGN KEY (`turn_id`) REFERENCES `conversation_turns` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_audit_events_agent_run_id`
        FOREIGN KEY (`agent_run_id`) REFERENCES `agent_runs` (`id`) ON DELETE SET NULL,
    CONSTRAINT `fk_audit_events_tool_call_id`
        FOREIGN KEY (`tool_call_id`) REFERENCES `tool_calls` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
